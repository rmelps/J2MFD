...
class EncounterManager {
    ...
    init(){
        ...
        for encounterFileName in encounterNames {
            ...
            // Turn labatts into molsons
            encounterNode.enumerateChildNodes(withName: "gold") {
                (node: SKNode, stop: UnsafeMutablePointer) in
                (node as? Beer)?.turnToMolson()
            }
        }
    }
    
    // addEncountersToScene function will be called from the GameScene to append
    ...
    func placeNextEncounter(currentXPos: CGFloat) {
        ...
        // Pick an encounter that is not currently on the screen
        var nextEncounterIndex: Int?
        var trulyNew: Bool?
        
        // The current encounter and the previous encounter
        // can potentially be on the screen at the same time
